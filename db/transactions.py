...
def add_transaction():
    ...
    tx_type = input("구분을 입력하세요  수입or지출 : ").strip()
    ...
    sql = """insert into TRANSACTIONS(tx_type,amount,memo,reg_date)
    values(:1,:2,:3,:4)
    """
...
def list_transaction():
    ...
    sql = "select * from TRANSACTIONS order by reg_date asc"
    cursor.execute(sql)
    rows = cursor.fetchall()

    ## todo 내용이 없는경우
    if not rows:
        print("등록된 가계부 내역은 없습니다.\n")
        return
    print("-"*50)
    for row in rows:
